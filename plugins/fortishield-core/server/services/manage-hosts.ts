/*
 * Fortishield app - Module to update the configuration file
...
import { initialFortishieldConfig } from './initial-fortishield-config';
import { FORTISHIELD_DATA_CONFIG_APP_PATH } from '../../common/constants';
...
export class ManageHosts {
  ...
  constructor(private logger: Logger, private updateRegistry: UpdateRegistry) {
    this.busy = false;
    this.file = FORTISHIELD_DATA_CONFIG_APP_PATH;
    this.initialConfig = initialFortishieldConfig;
  }
  ...
  /**
   * Returns the hosts in the fortishield.yml
   */
  async getHosts() {
    try {
      this.checkBusy();
      ...
      if (!fs.existsSync(FORTISHIELD_DATA_CONFIG_APP_PATH)) {
  ...
  /**
   * This function checks if the hosts: key exists in the fortishield.yml for preventing duplicate in case of there's not any host defined
   */
  ...
  /**
   * Returns the IDs of the current hosts in the fortishield.yml
   */
  ...
  async getHostById(id) {
    try {
      ...
      if (host && !host.length) {
        throw new Error('Selected API is no longer available in fortishield.yml');
      }
  ...
  /**
   *  Iterate the array with the API entries in given from the .fortishield index in order to create a valid array
  ...
  transformIndexedApis(apiEntries) {
    ...
    try {
      ...
      this.logger.debug('Transforming index API schedule to fortishield.yml');
    } catch (error) {
      this.logger.error(error.message || error);
      throw error;
    }
  ...
  /**
   * Receives an array of hosts and checks if any host is already in the fortishield.yml, in this case is removed from the received array and returns the resulting array
   * @param {Array} hosts
   */
  ...
  /**
   * Throws an error is the fortishield.yml is busy
   */
  checkBusy() {
    if (this.busy)
  ...
  async addSeveralHosts(hosts) {
    try {
      ...
      return 'All APIs entries were migrated to the fortishield.yml';
    } catch (error) {
      this.logger.error(error.message || error);
      return Promise.reject(error);
    }
  ...
  /**
   * Delete a host from the fortishield.yml
   * @param {Object} req
   */
