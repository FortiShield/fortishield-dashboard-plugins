 ...
 */
import fs from 'fs';
import { FORTISHIELD_DATA_CONFIG_REGISTRY_PATH } from '../../common/constants';
...
export class UpdateRegistry {
  ...
  constructor(private logger: Logger) {
    this.busy = false;
    this.file = FORTISHIELD_DATA_CONFIG_REGISTRY_PATH;
  }

  /**
   * Reads the Fortishield registry content
   */
  async readContent() {
    try {
  ...
  /**
   * Writes the fortishield-registry.json
   * @param {Object} content
   */
  async writeContent(content) {
    try {
      this.logger.debug('Writting fortishield-registry.json content');
      if (this.busy) {
  ...
  /**
   * Compare the hosts from fortishield.yml and the host in the fortishield-registry.json file in order to remove the orphan registry register
   * @param {Array} hosts
   */
