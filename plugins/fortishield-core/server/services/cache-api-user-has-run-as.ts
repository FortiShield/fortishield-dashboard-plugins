/*
 * Fortishield app - Service which caches the API user allow run as
...
export class CacheAPIUserAllowRunAs {
  ...
  async canUse(apiId: string): Promise<number | never> {
    ...
    if (ApiUserCanUseStatus === API_USER_STATUS_RUN_AS.USER_NOT_ALLOWED) {
      const api = await this.manageHosts.getHostById(apiId);
      throw new Error(
        `API with host ID [${apiId}] misconfigured. The Fortishield API user [${api.username}] is not allowed to use [run_as]. Allow it in the user configuration or set [run_as] host setting with [false] value.`,
      );
    }
