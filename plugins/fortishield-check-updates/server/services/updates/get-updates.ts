...
export const getUpdates = async (
  ...
  try {
    ...
    const { serverAPIHostEntries, api: fortishieldApiClient } = getFortishieldCore();
    ...
    const apisAvailableUpdates = await Promise.all(
      hosts?.map(async api => {
        ...
        try {
          const response = await fortishieldApiClient.client.asInternalUser.request(
            method,
            path,
            data,
            options,
    ...
    const { logger } = getFortishieldCheckUpdatesServices();
