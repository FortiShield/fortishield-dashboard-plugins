...
export function ApiProvider({ getService, getPageObjects }) {
  ...
  class ApiPage {
    ...
    async completeApiForm(type = 'apiConfig') {
      ...
      await testSubjects.setValue(`${type}Username`, 'fortishield');
      log.debug('insert the user');
      await testSubjects.setValue(`${type}Password`, 'fortishield');
    ...
    async checkTabDisabled(tab) {
      ...
      expect(await browser.getCurrentUrl()).to.contain('app/fortishield#/settings');
    }
