...
import {
	header,
	hostsConfiguration,
	initialFortishieldConfig,
	printSetting,
	printSettingCategory,
	printSettingValue,
	printSection,
} from './initial-fortishield-config';
...
describe('[configuration-file] Default configuration file content', () => {
	...
	it('Include the header', () => {
		expect(initialFortishieldConfig).toContain(header);
	});
	...
	it('Include all the expected categories and settings', () => {
		...
		pluginSettingsConfigurationFileGroupByCategory.forEach(({category,  settings}) => {
			// Category
			expect(initialFortishieldConfig).toContain(printSettingCategory(PLUGIN_SETTINGS_CATEGORIES[category]));

			// Category settings
			settings.forEach(setting => {
				expect(initialFortishieldConfig).toContain(printSetting(setting));
			});
		});
	...
	it('Include the host configuration', () => {
		expect(initialFortishieldConfig).toContain(hostsConfiguration);
	});	
});
