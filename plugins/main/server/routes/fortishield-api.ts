...
import { FortishieldApiCtrl } from '../controllers';
import { schema } from '@osd/config-schema';

export function FortishieldApiRoutes(router: IRouter) {
  const ctrl = new FortishieldApiCtrl();
  ...
  // Check if credentials on POST connect to Fortishield API. Not storing them!
  // Returns if the fortishield-api configuration received in the POST body will work
  router.post(
    {
      path: '/api/check-api',
      validate: {
  ...
  // Returns data from the Fortishield API on CSV readable format
  router.post(
    {
      path: '/api/csv',
      validate: {
  ...
  // Return Fortishield Appsetup info
  router.get(
    {
      path: '/api/setup',
      validate: false,
