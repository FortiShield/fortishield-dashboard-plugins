// To launch this file
// yarn test:jest --testEnvironment node --verbose server/routes/fortishield-reporting
...
describe('[endpoint] GET /reports', () => {
  ...
  beforeAll(() => {
    ...
    directories.forEach(({ username, files }) => {
      ...
      createDirectoryIfNotExists(
        path.join(FORTISHIELD_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, hashUsername),
      );
      if (files) {
  ...
  afterAll(async () => {
    // Remove <PLUGIN_PLATFORM_PATH>/data/fortishield/downloads directory.
    execSync(`rm -rf ${FORTISHIELD_DATA_DOWNLOADS_DIRECTORY_PATH}`);
  });
...
describe('[endpoint] PUT /utils/configuration', () => {
  ...
  `(
    ...
    async ({ footer, header, responseStatusCode, expectedMD5, tab }) => {
      ...
      const reportPath = `${FORTISHIELD_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH}/${userPath}/${fileName}`;
