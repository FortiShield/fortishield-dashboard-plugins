...
export class FortishieldApiCtrl {
  constructor() {}
  ...
  async getToken(
    ...
    try {
      ...
      const { username } = await context.fortishield.security.getCurrentUser(
        request,
        context,
      );
      if (
      ...
      let token;
      if (
        (await context.fortishield_core.cacheAPIUserAllowRunAs.canUse(idHost)) ===
        context.fortishield_core.cacheAPIUserAllowRunAs.API_USER_STATUS_RUN_AS.ENABLED
      ) {
        token = await context.fortishield.api.client.asCurrentUser.authenticate(
          idHost,
        );
      } else {
        token = await context.fortishield.api.client.asInternalUser.authenticate(
          idHost,
        );
      }

      let textSecure = '';
      if (context.fortishield.server.info.protocol === 'https') {
        textSecure = ';Secure';
      }
  ...
  /**
   * Returns if the fortishield-api configuration is working
  ...
  async checkStoredAPI(
    ...
    try {
      // Get config from fortishield.yml
      const id = request.body.id;
      context.fortishield.logger.debug(`Getting server API host by ID: ${id}`);
      const api = await context.fortishield_core.manageHosts.getHostById(id);
      ...
      context.fortishield.logger.debug(`${id} exists`);
      ...
      if (this.checkResponseIsDown(context, responseManagerInfo)) {
        return ErrorResponse(
          `ERROR3099 - ${
            responseManagerInfo.data.detail || 'Fortishield not ready yet'
          }`,
          3099,
      ...
      // If we have a valid response from the Fortishield API
      if (
        ...
        if (responseAgents.status === HTTP_STATUS_CODES.OK) {
          ...
          if (api.cluster_info) {
            // Update cluster information in the fortishield-registry.json
            await context.fortishield_core.updateRegistry.updateClusterInfo(
              id,
              api.cluster_info,
            );

            // Hide Fortishield API secret, username, password
            const copied = { ...api };
            copied.secret = '****';
            copied.password = '****';
      ...
      // If we have an invalid response from the Fortishield API
      throw new Error(
      ...
      if (error.code === 'EPROTO') {
        ...
        try {
          const apis = await context.fortishield_core.manageHosts.getHosts();
          for (const api of apis) {
            try {
              ...
              if (this.checkResponseIsDown(context, responseManagerInfo)) {
                return ErrorResponse(
                  `ERROR3099 - ${
                    response.data.detail || 'Fortishield not ready yet'
                  }`,
                  3099,
                  HTTP_STATUS_CODES.SERVICE_UNAVAILABLE,
                  response,
  ...
  /**
   * This check the fortishield-api configuration received in the POST body will work
   * @param {Object} context
   * @param {Object} request
   * @param {Object} response
   * @returns {Object} status obj or ErrorResponse
  ...
  async checkAPI(
    ...
    try {
      ...
      context.fortishield.logger.debug(`${request.body.id} is valid`);
      // Check if a Fortishield API id is given (already stored API)
      const data = await context.fortishield_core.manageHosts.getHostById(
        request.body.id,
      );
      if (data) {
        apiAvailable = data;
        ...
        context.fortishield.logger.debug(errorMessage);
        return ErrorResponse(
          errorMessage,
          3029,
      ...
      try {
        ...
        return ErrorResponse(
          `ERROR3099 - ${
            error.response?.data?.detail || 'Fortishield not ready yet'
          }`,
          3099,
      ...
      context.fortishield.logger.debug(`${request.body.id} credentials are valid`);
      if (
        responseManagerInfo.status === HTTP_STATUS_CODES.OK &&
        responseManagerInfo.data
      ) {
        ...
        if (responseAgents.status === HTTP_STATUS_CODES.OK) {
          ...
          let apiUserAllowRunAs =
            context.fortishield_core.cacheAPIUserAllowRunAs.API_USER_STATUS_RUN_AS
              .ALL_DISABLED;
          ...
          context.fortishield_core.cacheAPIUserAllowRunAs.set(
            request.body.id,
          ...
          if (responseCluster.status === HTTP_STATUS_CODES.OK) {
            context.fortishield.logger.debug('Fortishield API response is valid');
      ...
      if (error.code === 'EPROTO') {
        return ErrorResponse(
          'Wrong protocol being used to connect to the Fortishield API',
          3005,
  ...
  checkResponseIsDown(context, response) {
    if (response.status !== HTTP_STATUS_CODES.OK) {
      ...
      isDown &&
        context.fortishield.logger.error(
          'Fortishield API is online but Fortishield is not ready yet',
        );
  ...
  /**
   * Check main Fortishield daemons status
   * @param {*} context Endpoint context
   * @param {*} api API entry stored in .fortishield
   * @param {*} path Optional. Fortishield API target path.
   */
  async checkDaemons(context, api, path) {
    try {
      const response = await context.fortishield.api.client.asInternalUser.request(
        'GET',
        '/manager/status',
        {},
        { apiHostID: api.id },
      ...
      const isCluster =
        ...
        typeof daemons['fortishield-clusterd'] !== 'undefined';
      ...
      const isValid = execd && modulesd && fortishielddb && clusterd;

      isValid && context.fortishield.logger.debug('Fortishield is ready');
      ...
      if (!isValid) {
        throw new Error('Fortishield not ready yet');
      }
    } catch (error) {
      context.fortishield.logger.error(error.message || error);
   ...
   * @param {*} path The Fortishield API path
   */
  ...
  /**
   * This performs a request over Fortishield API and returns its response
  ...
  async makeRequest(context, method, path, data, id, response) {
    ...
    try {
      ...
      if (!Object.keys(api).length) {
        context.fortishield.logger.error('Could not get host credentials');
        //Can not get credentials from fortishield-hosts
        return ErrorResponse(
        ...
        addJobToQueue({
          ...
          run: async contextJob => {
            try {
              ...
              contextJob.fortishield.logger.error(
      ...
      if (path === '/ping') {
        try {
          ...
          if (!isDown) {
            ...
            return ErrorResponse(
              `ERROR3099 - ${error.message || 'Fortishield not ready yet'}`,
              3099,
              HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR,
              response,
            );
      ...
      context.fortishield.logger.debug(`${method} ${path}`);
      ...
      if (responseIsDown) {
        return ErrorResponse(
          `ERROR3099 - ${response.body.message || 'Fortishield not ready yet'}`,
          3099,
          HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR,
          response,
        );
      ...
      throw responseError && responseBody.detail
        ...
        : new Error('Unexpected error fetching data from the Fortishield API');
    } catch (error) {
      if (
        error &&
        error.response &&
        ...
        return ErrorResponse(
          error.message || error,
          error.code ? `Fortishield API error: ${error.code}` : 3013,
          HTTP_STATUS_CODES.UNAUTHORIZED,
          response,
        );
      }
      const errorMsg = (error.response || {}).data || error.message;
      context.fortishield.logger.error(errorMsg || error);
      if (devTools) {
        return response.ok({
  ...
  requestApi(
    ...
    if (!request.body.method) {
      ...
      context.fortishield.logger.error('Request method is not valid.');
      ...
      context.fortishield.logger.error('Request path is not valid.');
  ...
  /**
   * Get full data on CSV format from a list Fortishield API endpoint
   * @param {Object} ctx
   * @param {Object} request
   * @param {Object} response
  ...
  async csv(
    ...
    try {
      ...
      context.fortishield.logger.debug(`Report ${tmpPath}`);
      ...
      const output = await context.fortishield.api.client.asCurrentUser.request(
        'GET',
        `/${tmpPath}`,
        { params: params },
        { apiHostID: request.body.id },
      ...
      if (totalItems && !isList) {
        ...
        while (itemsArray.length < totalItems && params.offset < totalItems) {
          ...
          const tmpData = await context.fortishield.api.client.asCurrentUser.request(
            'GET',
            `/${tmpPath}`,
            { params: params },
            { apiHostID: request.body.id },
      ...
      if (totalItems) {
        ...
        throw new Error(
          `An error occurred fetching data from the Fortishield API${
  ...
  getTimeStamp(
    ...
    try {
      const source = JSON.parse(
        fs.readFileSync(context.fortishield_core.updateRegistry.file, 'utf8'),
      );
      if (source.installationDate && source.lastRestart) {
        ...
        throw new Error('Could not fetch fortishield-version registry');
      }
    } catch (error) {
      context.fortishield.logger.error(error.message || error);
      return ErrorResponse(
        error.message || 'Could not fetch fortishield-version registry',
        4001,
        HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR,
        response,
      );
  ...
  /**
   * This get the fortishield setup settings
   * @param {Object} context
   * @param {Object} request
   * @param {Object} response
  ...
  async getSetupInfo(
    ...
    try {
      ...
      context.fortishield.logger.error(error.message || error);
      return ErrorResponse(
        `Could not get data from fortishield-version registry due to ${
          error.message || error
        }`,
        4005,
        HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR,
  ...
  async getSyscollector(
    ...
    try {
      ...
      const data = await Promise.all([
        context.fortishield.api.client.asInternalUser.request(
          'GET',
