/*
 * Fortishield app - Class for Fortishield-Elastic functions
...
import {
  FORTISHIELD_ROLE_ADMINISTRATOR_ID,
  FORTISHIELD_SAMPLE_ALERTS_INDEX_SHARDS,
  FORTISHIELD_SAMPLE_ALERTS_INDEX_REPLICAS,
...
import {
  FORTISHIELD_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS,
  FORTISHIELD_SAMPLE_ALERTS_DEFAULT_NUMBER_ALERTS,
} from '../../common/constants';
import { getSettingDefaultValue } from '../../common/services/settings';
import { FORTISHIELD_INDEXER_NAME } from '../../common/constants';

export class FortishieldElasticCtrl {
  ...
  async getTemplate(
    ...
    try {
      ...
      if (!templates || typeof templates !== 'string') {
        throw new Error(
          `An unknown error occurred when fetching templates from ${FORTISHIELD_INDEXER_NAME}`,
        );
      }
      ...
      return ErrorResponse(
        `Could not retrieve templates from ${FORTISHIELD_INDEXER_NAME} due to ${
          error.message || error
        }`,
        4002,
        500,
  ...
  async getCurrentPlatform(
    ...
    try {
      return response.ok({
        body: {
          platform: context.fortishield.security.platform,
        },
      });
    } catch (error) {
      context.fortishield.logger.error(error.message || error);
   ...
   * @param {String} id Index-pattern id to use in the visualizations. Eg: 'fortishield-alerts'
   */
  buildVisualizationsRaw(context, app_objects, id, namespace = false) {
    const config = getConfiguration();
    let monitoringPattern =
      (config || {})['fortishield.monitoring.pattern'] ||
      getSettingDefaultValue('fortishield.monitoring.pattern');
    context.fortishield.logger.debug(`Building ${app_objects.length} visualizations`);
    context.fortishield.logger.debug(`Index pattern ID: ${id}`);
    const visArray = [];
    ...
    for (let element of app_objects) {
      ...
      if (
        ...
        const isMonitoring = defaultStr.includes('fortishield-monitoring');
        if (isMonitoring) {
          ...
          aux_source.kibanaSavedObjectMeta.searchSourceJSON =
            defaultStr.replace(/fortishield-alerts/g, id);
        }
      }

      // Replace index-pattern for selector visualizations
      if (typeof (aux_source || {}).visState === 'string') {
        aux_source.visState = aux_source.visState.replace(/fortishield-alerts/g, id);
      }
   ...
   * @param {String} name Cluster name. Eg: 'fortishield'
  ...
  buildClusterVisualizationsRaw(
    ...
    try {
      ...
      for (const element of app_objects) {
        ...
        aux_source = aux_source.replace(/fortishield-alerts/g, id);
        ...
        if (title.startsWith('Fortishield App Statistics')) {
          const filter =
  ...
  async haveSampleAlerts(
    ...
    try {
      // Check if fortishield sample alerts index exists
      const results = await Promise.all(
        Object.keys(FORTISHIELD_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS).map(category =>
  ...
  /**
   * This creates sample alerts in fortishield-sample-alerts
  ...
  async createSampleAlerts(
    ...
    try {
      ...
      if (
        ...
        !decodedToken.rbac_roles.includes(FORTISHIELD_ROLE_ADMINISTRATOR_ID)
      ) {
      ...
      const sampleAlerts = FORTISHIELD_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS[
        request.params.category
      ]
        .map(typeAlert =>
          generateAlerts(
      ...
      if (!existsSampleIndex.body) {
        // Create fortishield sample alerts index

        const configuration = {
          settings: {
            index: {
              number_of_shards: FORTISHIELD_SAMPLE_ALERTS_INDEX_SHARDS,
              number_of_replicas: FORTISHIELD_SAMPLE_ALERTS_INDEX_REPLICAS,
            },
          },
        };
        ...
        context.fortishield.logger.info(`Index ${sampleAlertsIndex} created`);
      }
  ...
  async deleteSampleAlerts(
    ...
    // Delete Fortishield sample alert index
    ...
    try {
      ...
      if (existsSampleIndex.body) {
        // Delete Fortishield sample alerts index
        await context.core.opensearch.client.asCurrentUser.indices.delete({
          index: sampleAlertsIndex,
        });
        context.fortishield.logger.info(`Deleted ${sampleAlertsIndex} index`);
        return response.ok({
  ...
  async existStatisticsIndices(
    ...
    try {
      const config = getConfiguration();
      const statisticsPattern = `${config['cron.prefix'] || 'fortishield'}-${
        config['cron.statistics.index.name'] || 'statistics'
      }*`; //TODO: replace by default as constants instead hardcoded ('fortishield' and 'statistics')
      const existIndex =
        await context.core.opensearch.client.asCurrentUser.indices.exists({
