...
import { FortishieldReportingRoutes } from '../routes/fortishield-reporting';
import md5 from 'md5';
import {
...
beforeAll(async () => {
  ...
  // Create report files
  [
    { name: md5('admin'), files: ['fortishield-module-overview-general-1234.pdf'] },
    {
      name: md5('../../etc'),
      files: ['fortishield-module-overview-general-1234.pdf'],
    },
  ].forEach(({ name, files }) => {
    createDirectoryIfNotExists(
...
describe('[endpoint][security] GET /reports/{name} - Parameters validation', () => {
  it.each`
    ...
    ${'Get report by filename'} | ${'admin'}     | ${'fortishield-module-overview-general-1234.pdf'}               | ${200}             | ${null}
    ${'Invalid parameters'}     | ${'admin'}     | ${'..%2ffortishield-module-overview-general-1234.pdf'}          | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'}     | ${'admin'}     | ${'custom..%2ffortishield-module-overview-general-1234.pdf'}    | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}        | ${'admin'}     | ${'../custom..%2ffortishield-module-overview-general-1234.pdf'} | ${404}             | ${/Not Found/}
    ${'Get report by filename'} | ${'../../etc'} | ${'fortishield-module-overview-general-1234.pdf'}               | ${200}             | ${null}
    ${'Invalid parameters'}     | ${'../../etc'} | ${'..%2ffortishield-module-overview-general-1234.pdf'}          | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'}     | ${'../../etc'} | ${'custom..%2ffortishield-module-overview-general-1234.pdf'}    | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}        | ${'../../etc'} | ${'../custom..%2ffortishield-module-overview-general-1234.pdf'} | ${404}             | ${/Not Found/}
  `(
    ...
    async ({
      ...
      const response = await supertest(innerServer.listener)
        ...
        .send({
          ...
          tables: [],
          section: 'overview',
          indexPatternTitle: 'fortishield-alerts-*',
          apiId: 'default',
        })
        .expect(responseStatusCode);
      if (responseBodyMessage) {
...
describe('[endpoint][security] DELETE /reports/{name} - Parameters validation', () => {
  it.each`
    testTitle               | username       | filename                                               | responseStatusCode | responseBodyMessage
    ${'Delete report file'} | ${'admin'}     | ${'fortishield-module-overview-general-1234.pdf'}            | ${200}             | ${null}
    ${'Invalid parameters'} | ${'admin'}     | ${'..%2ffortishield-module-overview-general-1234.pdf'}       | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'} | ${'admin'}     | ${'custom..%2ffortishield-module-overview-general-1234.pdf'} | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}    | ${'admin'}     | ${'../fortishield-module-overview-general-1234.pdf'}         | ${404}             | ${/Not Found/}
    ${'Delete report file'} | ${'../../etc'} | ${'fortishield-module-overview-general-1234.pdf'}            | ${200}             | ${null}
    ${'Invalid parameters'} | ${'../../etc'} | ${'..%2ffortishield-module-overview-general-1234.pdf'}       | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'} | ${'../../etc'} | ${'custom..%2ffortishield-module-overview-general-1234.pdf'} | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}    | ${'../../etc'} | ${'../fortishield-module-overview-general-1234.pdf'}         | ${404}             | ${/Not Found/}
  `(
