/*
 * Fortishield app - Class for Fortishield reporting controller
...
export class FortishieldReportingCtrl {
  constructor() {}
  /**
    ...
    try {
      ...
      if (section && typeof section === 'string') {
        if (!['agentConfig', 'groupConfig'].includes(section)) {
          printer.addContent({
            text: FORTISHIELD_MODULES[tab].title + ' report',
            style: 'h1',
          });
      ...
      if (FORTISHIELD_MODULES[tab] && FORTISHIELD_MODULES[tab].description) {
        printer.addContentWithNewLine({
          text: FORTISHIELD_MODULES[tab].description,
          style: 'standard',
        });
      }
    } catch (error) {
  ...
  private getConfigRows(context, data, labels) {
    context.fortishield.logger.debug('Building configuration rows');
    const result = [];
  ...
  private getConfigTables(context, data, section, tab, array = []) {
    context.fortishield.logger.debug('Building configuration tables');
    let plainData = {};
    const nestedData = [];
    const tableData = [];
  ...
  createReportsModules = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.fortishield.logger.debug('Report started');
        const {
          array,
          agents,
          browserTimezone,
        ...
        const printer = new ReportPrinter(
          context.fortishield.logger.get('report-printer'),
        );
        ...
        createDirectoryIfNotExists(
          path.join(
            FORTISHIELD_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
            context.fortishieldEndpointParams.hashUsername,
          ),
        );
        ...
        await printer.print(context.fortishieldEndpointParams.pathFilename);

        return response.ok({
          body: {
            success: true,
            message: `Report ${context.fortishieldEndpointParams.filename} was created`,
          },
        });
      } catch (error) {
  ...
  createReportsGroups = this.checkReportsUserDirectoryIsValidRouteDecorator(
    ...
    ({ params: { groupID } }) =>
      `fortishield-group-configuration-${groupID}-${this.generateReportTimestamp()}.pdf`,
  );
  ...
  createReportsAgentsConfiguration =
    this.checkReportsUserDirectoryIsValidRouteDecorator(
      async (
        ...
        try {
          ...
          try {
            ...
            context.fortishield.logger.debug(error.message || error);
          }
      ...
      ({ params: { agentID } }) =>
        `fortishield-agent-configuration-${agentID}-${this.generateReportTimestamp()}.pdf`,
    );
    ...
    this.checkReportsUserDirectoryIsValidRouteDecorator(
      async (
        ...
        try {
          ...
          const { hashUsername } = await context.fortishield.security.getCurrentUser(
            request,
            context,
          );
          createDataDirectoryIfNotExists();
          ...
          context.fortishield.logger.debug('Syscollector report');
          const [sanitizedFilters, agentsFilter] = filters
            ? this.sanitizeKibanaFilters(context, filters, searchBar)
            : [false, null];
          ...
          const requestInventory = async agentRequestInventory => {
            try {
              context.fortishield.logger.debug(agentRequestInventory.loggerMessage);
      ...
      ({ params: { agentID } }) =>
        `fortishield-agent-inventory-${agentID}-${this.generateReportTimestamp()}.pdf`,
    );
  ...
  async getReports(
    ...
    try {
      context.fortishield.logger.debug('Fetching created reports');
      const { hashUsername } = await context.fortishield.security.getCurrentUser(
        request,
        context,
      );
      ...
      context.fortishield.logger.debug(`Directory: ${userReportsDirectoryPath}`);
      ...
      context.fortishield.logger.debug(`Total reports: ${reports.length}`);
      return response.ok({
        body: { reports },
      });
    } catch (error) {
  ...
  getReportByName = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.fortishield.logger.debug(
          `Getting ${context.fortishieldEndpointParams.pathFilename} report`,
        );
        const reportFileBuffer = fs.readFileSync(
          context.fortishieldEndpointParams.pathFilename,
        );
        return response.ok({
  ...
  deleteReportByName = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.fortishield.logger.debug(
          `Deleting ${context.fortishieldEndpointParams.pathFilename} report`,
        );
        fs.unlinkSync(context.fortishieldEndpointParams.pathFilename);
        context.fortishield.logger.info(
          `${context.fortishieldEndpointParams.pathFilename} report was deleted`,
        );
        return response.ok({
          body: { error: 0 },
        });
    ...
    return async (
      ...
      try {
        const { username, hashUsername } =
          await context.fortishield.security.getCurrentUser(request, context);
        const userReportsDirectoryPath = path.join(
          FORTISHIELD_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
          hashUsername,
        );
        ...
        return await routeHandler.bind(this)(
          {
            ...context,
            fortishieldEndpointParams: { hashUsername, filename, pathFilename },
          },
          request,
          response,
        );
