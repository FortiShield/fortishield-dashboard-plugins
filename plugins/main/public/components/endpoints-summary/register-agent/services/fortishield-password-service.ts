...
export const osdfucatePasswordInCommand = (password: string, commandText: string, os: tOperatingSystem['name']): string => {
  ...
  switch (osName){
    case 'macos':
    {
      const regex = /FORTISHIELD_REGISTRATION_PASSWORD=\'((?:\\'|[^']|[\"'])*)'/g;
    ...
    case 'windows':
      {
        const replacedString = command.replace(
          `FORTISHIELD_REGISTRATION_PASSWORD=\'${scapeSpecialCharsForWindows(password)}'`,
          () => {
            return `FORTISHIELD_REGISTRATION_PASSWORD=\'${'*'.repeat(scapeSpecialCharsForWindows(password).length)}\'`;
          }
        );
        return replacedString;
      }
    ...
    {
      const replacedString = command.replace(
        `FORTISHIELD_REGISTRATION_PASSWORD=\$'${scapeSpecialCharsForLinux(password)}'`,
        () => {
          return `FORTISHIELD_REGISTRATION_PASSWORD=\$\'${'*'.repeat(scapeSpecialCharsForLinux(password).length)}\'`;
        }
      );
      return replacedString;
    }
