/*
 * Fortishield app - Services related to agents
...
export function getAgentOSType(agent) {
  if (agent?.os?.uname?.toLowerCase().includes(FORTISHIELD_AGENTS_OS_TYPE.LINUX)) {
    return FORTISHIELD_AGENTS_OS_TYPE.LINUX;
  } else if (agent?.os?.platform === FORTISHIELD_AGENTS_OS_TYPE.WINDOWS) {
    return FORTISHIELD_AGENTS_OS_TYPE.WINDOWS;
  } else if (agent?.os?.platform === FORTISHIELD_AGENTS_OS_TYPE.SUNOS) {
    return FORTISHIELD_AGENTS_OS_TYPE.SUNOS;
  } else if (agent?.os?.platform === FORTISHIELD_AGENTS_OS_TYPE.DARWIN) {
    return FORTISHIELD_AGENTS_OS_TYPE.DARWIN;
  } else {
    return FORTISHIELD_AGENTS_OS_TYPE.OTHERS;
  }
}
