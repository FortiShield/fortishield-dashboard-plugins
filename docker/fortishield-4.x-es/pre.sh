...
usage() {
	echo
	echo "./pre.sh elastic_version fortishield_api_version action "
	echo
	echo "where"
	echo "  elastic_version is one of " ${elastic_versions[*]}
	echo "  fortishield_api_version is the version of fortishield, for example " ${fortishield_api_version[*]}
...
if [ $? -ne 0 ]; then
	echo "Version ${2} not found in ${fortishield_api_version[*]}"
	exit -1
fi
...
cat <<EOF >config/imposter/api_info.json
{
  "data": {
    ...
    "api_version": "${fortishield_version}",
    "revision": 40316,
    "license_name": "GPL 2.0",
    "license_url": "https://github.com/fortishield/fortishield/blob/4.3/LICENSE",
    "hostname": "imposter",
...
up)
	...
	echo "docker cp fortishield_kibana-${fortishield_version}_${ES_VERSION}-1.zip ${COMPOSE_PROJECT_NAME}-kibana-1:/tmp"
	echo
	echo "2. Install the pre-release package:"
	echo "docker exec -ti ${COMPOSE_PROJECT_NAME}-kibana-1 /usr/share/kibana/bin/kibana-plugin install file:///tmp/fortishield_kibana-${fortishield_version}_${ES_VERSION}-1.zip"
	echo
	echo "3. Restart Kibana:"
	echo "docker restart ${COMPOSE_PROJECT_NAME}-kibana-1"
	echo
	echo "4. Upload the Fortishield app configuration:"
