...
usage() {
	...
	echo "  fortishield_api_version is the patch version of fortishield 4.4, for example " ${fortishield_api_version[*]}
...
cat <<EOF >config/imposter/api_info.json
{
  "data": {
    ...
    "license_url": "https://github.com/fortishield/fortishield/blob/4.4/LICENSE",
    "hostname": "imposter",
...
up)
	...
	echo docker cp fortishield-4.4.${patch_version}-1.zip ${COMPOSE_PROJECT_NAME}-fortishield.dashboard-1:/tmp
	echo
	echo "4. Install the package we have just uploaded:"
	echo "docker exec -ti  ${COMPOSE_PROJECT_NAME}-fortishield.dashboard-1  /usr/share/fortishield-dashboard/bin/opensearch-dashboards-plugin install file:///tmp/fortishield-4.4.${patch_version}-1.zip"
	echo
