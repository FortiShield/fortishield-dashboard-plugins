...
usage() {
	...
	echo "  fortishield_api_version is the patch version of fortishield 4.3, for example " ${fortishield_api_version[*]}
...
case "$3" in
	up)
		...
		echo docker cp fortishield-4.3.${patch_version}-1.zip ${COMPOSE_PROJECT_NAME}-fortishield.dashboard-1:/tmp
		echo
		echo "4. Install the package we have just uploaded:"
		echo "docker exec -ti  ${COMPOSE_PROJECT_NAME}-fortishield.dashboard-1  /usr/share/fortishield-dashboard/bin/opensearch-dashboards-plugin install file:///tmp/fortishield-4.3.${patch_version}-1.zip"
		echo
